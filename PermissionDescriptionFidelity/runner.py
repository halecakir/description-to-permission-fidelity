...
def parse_arguments():
    ...
    parser.add_argument(
        "--train-type", dest="train_file_type", help="Train file type", default="csv"
    )
    parser.add_argument(
        "--test", dest="test", help="Path to test file", metavar="FILE", default="N/A"
    )
    parser.add_argument(
        "--test-type", dest="test_file_type", help="Test file type", default="csv"
    )
    parser.add_argument(
        "--prevectors",
    ...
    parser.add_argument(
        "--lstm-type", dest="lstm_type", help="lstm or bilstm", default="lstm"
    )
    parser.add_argument("--wembedding", type=int, dest="wembedding_dims", default=300)
    ...
    parser.add_argument(
        "--stemmer", dest="stemmer", help="Apply word stemmer", default="no_stemmer"
    )
    parser.add_argument(
        "--external-info",
        dest="external_info",
...
def main():
    ...
    w2i, permissions = IOUtils.load_vocab(
        args.test,
        args.test_file_type,
    ...
    train_data, _ = IOUtils.train_test_split(
        args.train, args.train_file_type, args.sequence_type, args.window_size
    )
...
def call_similarity_experiment():
    ...
    args = parse_arguments()
    print("Extracting vocabulary")
    train_w2i, _ = IOUtils.load_vocab(
        args.train,
