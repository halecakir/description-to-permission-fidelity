            ...
            for row in reader:
                ...
                for sentence in Utils.preprocess(text):
                    ...
                    for p in  row[2].strip().split(","):
                        permsCount.update([Utils.to_lower(p, lower)])
                        for token in p.split("."):
                            wordsCount.update([Utils.to_lower(token, lower)])
        return wordsCount.keys(), {w: i for i, w in enumerate(list(wordsCount.keys()))}, {p: i for i, p in enumerate(list(permsCount.keys()))}

    def read_csv(file_path, w2i, p2i, lower=True):
        data = []
        doc_id = 0
        with open(file_path) as f:
            reader = csv.reader(f)
            ...
            for row in reader:
                ...
                for p in  row[2].strip().split(","):
                    ...
                    pphrase = [Utils.to_lower(p, lower) for t in p.split(".")]
